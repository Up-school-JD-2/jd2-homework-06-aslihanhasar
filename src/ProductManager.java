...
public class ProductManager {
    ...
    public List<Product> getActiveProductsSortedByPrice() {
        Predicate<Product> isActiveStatus = product -> product.getProductStatus() == ProductStatus.ACTIVE;
    ...
    //kategorideki ürünlerin sayısına bölerek ortalama alır
    public double calculateAveragePriceInCategory2(String category) {
        Predicate<Product> categoryPredicate = product -> product.getCategory().equalsIgnoreCase(category);
    ...
    //kategorideki stok sayısına bölerek ortalama alır
    public double calculateAveragePriceInCategory(String category) {
        ...
        double totalPrice = products.values().stream()
                ...
                .sum();
        long productCount = products.values().stream()
                .filter(categoryPredicate)
                .mapToLong(Product::getStock)
                .sum();
        OptionalDouble averagePrice;
        if (productCount > 0) {
            averagePrice = OptionalDouble.of(totalPrice / productCount);
        } else {
            averagePrice = OptionalDouble.empty();
        }
        return averagePrice.orElse(0.0);
    }
